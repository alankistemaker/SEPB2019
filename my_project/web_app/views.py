...
from web_app.models import Proposal, Project, Client
...
def proposal_list(request):
    ...
    if filter_value:
        proposal_filter = proposal_list.filter(Q(proposal_id__icontains=filter_value) | Q(proposal_name__icontains=filter_value))
    else:
...
def project_list(request):
    ...
    if filter_value:
        project_filter = project_list.filter(Q(project_id__icontains=filter_value) | Q(project_name__icontains=filter_value) | Q(project_category__icontains=filter_value))
    else:
        project_filter = project_list.all()
    return render(request, 'project_list.html', {"project_filter": project_filter, "filter_value": filter_value})
...
def project_detail(request, pk=None):
    ...
    if request.method == "POST":
        ...
        if request.POST.get("save") == "save":
            project_detail = Project.objects.filter(project_id=project_id).update(project_name=project_name, project_category=project_category, project_year=project_year, 
                                                    project_groupname=project_groupname, project_unit=project_unit, project_convenor=project_convenor, 
                                                    project_supervisor=project_supervisor, project_teamleader=project_teamleader, project_groupsize=project_groupsize)
...
def new_client(request):   
    if request.method == "POST":
        ...
        if request.POST.get("save") == "save":
            new_client = Client.objects.create(client_id=client_id, client_name=client_name, client_address=client_address, 
                                               client_website=client_website, client_contact=client_contact, client_title=client_title, 
...
def client_list(request):
    ...
    if filter_value:
        client_filter = client_list.filter(Q(client_id__icontains=filter_value) | Q(client_name__icontains=filter_value))
    else:
...
def client_detail(request, pk=None):
    ...
    if request.method == "POST":
        ...
        if request.POST.get("save") == "save":
            client_detail = Client.objects.filter(client_id=client_id).update(client_name=client_name, client_address=client_address, 
                                                  client_website=client_website, client_contact=client_contact, client_title=client_title, 
                                                  client_phone=client_phone, client_email=client_email)
